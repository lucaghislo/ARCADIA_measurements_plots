...
# Optimal R0
optimal_R0_R2 = pd.read_csv(r"output\plot_6\mean_slope_optimized_R0.dat", sep="\t")
print(optimal_R0_R2)
...
plt.xlabel(r"Temperature [$^{\circ}$C]")
plt.ylabel(r"$V_{OUT}$ [V]")
plt.title(r"\textbf{Mean} \boldmath$V_{OUT}$ \textbf{vs temperature (optimal R0)}")
plt.grid()

print_plot(
    output_folder_drive,
    output_folder_github,
    "plot_9_optimized_R0.pdf",
)


# Optimal R2
optimal_R0_R2 = pd.read_csv(r"output\plot_6\mean_slope_optimized_R2.dat", sep="\t")
print(optimal_R0_R2)
...
plt.xlabel(r"Temperature [$^{\circ}$C]")
plt.ylabel(r"$V_{OUT}$ [V]")
plt.title(r"\textbf{Mean} \boldmath$V_{OUT}$ \textbf{vs temperature (optimal R2)}")
plt.grid()

print_plot(
    output_folder_drive,
    output_folder_github,
    "plot_9_optimized_R2.pdf",
)
...
# Optimal R0 and R2 as to get Vout closest to 600 mV
optimal_R0_R2 = pd.read_csv(
    r"output\plot_6\mean_slope_optimized_R0_nearest_R2.dat", sep="\t"
)
print(optimal_R0_R2)
...
plt.ylabel(r"$V_{OUT}$ [V]")
plt.title(
    r"\textbf{Mean} \boldmath$V_{OUT}$ \textbf{vs temperature (R2 nearest to 600 mV)}"
)
plt.grid()
...
print_plot(
    output_folder_drive,
    output_folder_github,
    "plot_9_optimized_R0_nearest_R2.pdf",
)
