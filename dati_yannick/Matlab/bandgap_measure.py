...
# Pop-up
top = Tk()
top.geometry(f"200x50+{800}+{400}") # size and position of window
...
#volt_in = np.arange(0,1.33,0.01)
volt_in = np.arange(1.32,-0.01, -0.01)
...
if tmp < 0:
    temper = tmp*-1
    results.to_csv('/home/microlab/Documenti/Python_scripts/Band_gap/VIN_VOUT/Results_TP'+str(TP)+'_TEMP_m'+str(temper)+'_132_DOWN.csv')
else:
    results.to_csv('/home/microlab/Documenti/Python_scripts/Band_gap/VIN_VOUT/Results_TP'+str(TP)+'_TEMP_'+str(tmp)+'_132_DOWN.csv')
