...
i = np.arange(0, 3, 1)
print(ps_volts)
reg = np.arange(0,16,1)

#####################################
#must be manually changed
TP = 1
tmp = 30
filename = 'Results_TP_1_REG_30_0_01.csv'
#####################################
...
for n in i:
    ...
    for register in reg:
        register_mean = 'BIAS'+str(register)+'_BGR_MEAN' %register
        register_slope = 'BIAS'+str(register)+'_BGR_SLOPE' %register 
